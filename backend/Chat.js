...
function chat(io) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join-room', ({ rid, uid }) => {
            // console.log(rid,uid)
            db.User.findById(uid, 'interactions').populate({ path: 'interactions', populate: [{ path: 'otherUser', select: 'name photo _id email' }, { path: 'conversation', select: 'updatedAt _id' }] })
                .then(async (result) => {
                    ...
                    if (otherUser) {
                        ...
                        db.Conversation.findById(rid).populate({ path: 'messages', populate: { path: "author" } })
                            .then(async a => {
                                try {
                                    let otherUserPopulated = await db.User.findById(otherUser.otherUser, 'name email _id photo')
                    ...
                    .then(async a => {
                        if (a.messages.length == 0) {
                            try {
                                ...
                                let currentUser = await db.User.findById(data.uid, '_id name emial photo')
